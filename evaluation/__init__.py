...
def evaluate(dataset, LOG, metric_computer, dataloaders, model, opt, evaltypes, device,
             ...
             compute_metrics_only=False, print_text=True):
    """
    Parent-Function to compute evaluation metrics, print summary string and store checkpoint files/plot sample recall plots.
    """
    computed_metrics, extra_infos = metric_computer.compute_standard(opt, model, dataloaders[0], evaltypes, device, mode=log_key)
    ...
    if not compute_metrics_only:
        ...
        for evaltype in evaltypes:
            for storage_metric in opt.storage_metrics:
                ...
                ref_mets = LOG.progress_saver[log_key].groups[parent_metric][storage_metric]['content']
                if not len(ref_mets): ref_mets = [-np.inf]
                if numeric_metrics[evaltype][storage_metric]>np.max(ref_mets):
                   print('Saved improved checkpoint for {}: {}\n'.format(log_key, parent_metric))
                   set_checkpoint(model, opt, LOG, LOG.prop.save_path+'/checkpoint_{}_{}_{}.pth.tar'.format(log_key, evaltype, storage_metric), aux=aux_store)
    else:
